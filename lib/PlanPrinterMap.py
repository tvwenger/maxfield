...
class PlanPrinter:
    def __init__(self,a,outputDir,nagents,color='#FF004D'):
        ...
        latmax = max([self.a.node[i]['geo'][0] for i in self.a.node.keys()])*180./3.141592654
        latmin = min([self.a.node[i]['geo'][0] for i in self.a.node.keys()])*180./3.141592654
        lonmax = max([self.a.node[i]['geo'][1] for i in self.a.node.keys()])*180./3.141592654
        lonmin = min([self.a.node[i]['geo'][1] for i in self.a.node.keys()])*180./3.141592654
        self.loncenter = (lonmax-lonmin)/2. + lonmin
        self.latcenter = (latmax-latmin)/2. + latmin
...
### The code below works. It just uses networkx draw functions
    ...
    def planMap(self):
        ...
        url = "http://maps.googleapis.com/maps/api/staticmap?center={0},{1}&size=640x640&zoom=14&sensor=false".format(self.loncenter,self.latcenter)
